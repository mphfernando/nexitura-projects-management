...
export default function ProjectShell({ project, initialTab, onBackToProjects, onOpenAdmin, onNavigate }) {
  ...
  // Reviewers (Admin/Super Admin) see the Bugs tab framed as what it is for
  // them — reviewing what Clients reported — rather than an invitation to submit.
  const tabLabel = t => (t === "bugs" && isUnrestricted) ? "Reported Bugs by Client" : TAB_LABELS[t];
  const tabShortLabel = t => (t === "bugs" && isUnrestricted) ? "Bugs" : TAB_LABELS[t].split(" ")[0];
    ...
    <div className="min-h-screen pb-16 md:pb-0">
      <header className="relative bg-[var(--ink)] text-white px-5 py-3.5 flex flex-wrap items-center gap-x-4 gap-y-1">
        ...
        <div className="flex items-center gap-2">
          <div className="bg-white rounded-lg px-1.5 py-1 flex items-center"><img src="/logo.jpeg" alt="Nexitura" className="h-5 w-auto" /></div>
