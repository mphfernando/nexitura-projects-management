    ...
    <div>
      ...
      <Card>
        ...
        {sorted.length === 0 ? <EmptyState>No milestones yet.</EmptyState> : sorted.map(m => (
          <div key={m.id} className="flex gap-3 items-start py-3.5 border-b border-dashed border-[var(--line)] last:border-none flex-wrap">
            ...
            <div className="flex gap-1.5 items-center shrink-0 ml-auto">
              ...
              <Badge tone={stTone(m.status)} onClick={() => cycleStatus(m.id)}>{m.status}</Badge>
              <button onClick={() => del(m.id)} className="text-[var(--muted)] hover:bg-[var(--red-soft)] hover:text-[var(--red)] rounded-md px-2 py-1 text-lg leading-none">×</button>
            </div>
          </div>
        ))}
      </Card>
